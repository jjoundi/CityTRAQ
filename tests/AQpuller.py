...
if response.status_code == 200:
    ...
    for hour in sorted(hourly_averages.keys()):
        ...
        new_row = [f"hour_avg {hour}", "Krekelberg", hourly_timestamps[hour], datetime.utcfromtimestamp(hourly_timestamps[hour] / 1000).strftime('%Y-%m-%d %H:%M:%S'), hourly_averages[hour]]
        transformed_data.append(new_row)
...
if response.status_code == 200:
    ...
    for hour in sorted(hourly_averages.keys()):
        ...
        new_row = [f"hour_avg {hour}", "Louis Schuerman", hourly_timestamps[hour], datetime.utcfromtimestamp(hourly_timestamps[hour] / 1000).strftime('%Y-%m-%d %H:%M:%S'), hourly_averages[hour]]
        transformed_data.append(new_row)
